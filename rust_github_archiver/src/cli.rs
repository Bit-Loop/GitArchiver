...
impl CliApp {
    ...
    pub fn run() -> Result<()> {
        ...
        match matches.subcommand() {
            ...
            Some(("status", _)) => tokio::runtime::Runtime::new()?.block_on(app.show_status()),
            Some(("cleanup", _)) => tokio::runtime::Runtime::new()?.block_on(app.cleanup()),
            _ => {
    ...
    fn build_cli() -> Command {
        Command::new("github_archiver")
            ...
            .subcommand(Command::new("cleanup").about("Clean up old files and resources"))
    }
    ...
    async fn run_server(&mut self, matches: &ArgMatches) -> Result<()> {
        ...
        let server = ApiServer::new(self.config.clone()).await?;
        server.start().await?;
            ...
            for file in files_to_process {
                ...
                match main_scraper.process_single_file(&file.filename).await {
                    Ok(result) => {
                        info!(
                            "✓ {}: {} events processed",
                            file.filename, result.valid_events
                        );
                    }
                    Err(e) => {
        ...
        match main_scraper.get_comprehensive_status().await {
            Ok(status) => {
                ...
                if let Some(resource_status) = status.resource_status {
                    ...
                    info!(
                        "  Memory: {:.1} GB ({:.1}%)",
                        resource_status.memory.used_gb, resource_status.memory.percent
                    );
                    info!(
                        "  Disk: {:.1} GB ({:.1}%)",
                        resource_status.disk.used_gb, resource_status.disk.percent
                    );
